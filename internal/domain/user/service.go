...
func (s *service) Login(ctx context.Context, loginCtx LoginContext) (*LoginResponse, error) {
	...
	accessToken, accessPayload, err := s.tokenMaker.CreateToken(user.ID, user.Username, "user", s.config.JWT.AccessTokenDuration)
	if err != nil {
		return nil, fmt.Errorf("failed to create access token: %w", err)
	}

	refreshToken, refreshPayload, err := s.tokenMaker.CreateToken(user.ID, user.Username, "user", s.config.JWT.RefreshTokenDuration)
	if err != nil {
	...
	_ = s.repo.UpdateUserLastLogin(ctx, sqlc.UpdateUserLastLoginParams{
		LastLoginAt: types.NullTime{Time: time.Now(), Valid: true},
...
func (s *service) UpdateProfile(ctx context.Context, userID int64, req UpdateProfileRequest) (*UserResponse, error) {
	...
	err = s.repo.UpdateUser(ctx, sqlc.UpdateUserParams{
		...
		Avatar:   req.Avatar,
		Gender:   req.Gender,
		Birthday: types.NewNullTime(req.Birthday),
		ID:       userID,
	})
	if err != nil {
